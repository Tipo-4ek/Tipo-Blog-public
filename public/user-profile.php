      ...
      ?>
        ...
        <div class="col-md-9" id = "color2">
          <div class="row">
             ...
             <div class="col-md-5">
              ...
              <p class='h6' style = "padding-left: 48px;">
               ...
               <ul class="menu mt-5">
                  <!-- Карма, подписота -->
                  <li> Карма <br><a title = "Статистика кармы" href="#"><img src="<?= __DIR_PUBLIC__.'/media/img/1.png'?>"></a><br><?php
                   
                   if ($view_person_data['person_karma'] < 0)
                   {
                    $view = "<span style='color:red;'>".$view_person_data['person_karma']."</span>";
                   }
                   else
                    $view = $view_person_data['person_karma'];
                  echo $view;
                   ?></li>
          ...
          <nav class="nav nav-pills flex-column flex-sm-row justify-content-center">
               ...
               <a class="flex-sm-fill text-sm-center nav-link active" title = "Профиль пользователя" href="<?= __DIR_PUBLIC__.'/user-profile.php?id='.$_GET['id']?>">Профиль</a>
               ...
               <a class="flex-sm-fill text-sm-center nav-link" title = "Статьи, которые пользователь добавил в закладки" href="<?= __DIR_PUBLIC__.'/user-bookmarks.php?id='.$_GET['id']?>">Закладки</a>         
          </nav>
          <br>
          <div class ='row col-md-10 d-none d-md-block'>
            <p class = 'profile-section__title pl-1' > О себе: </p>
            <p class = 'pl-2 profile-section__about-text'>
              <?php
                if (isset($view_person_data['person_status_about']) and $view_person_data['person_status_about']!='')
                {
                  echo $view_person_data['person_status_about'];
                }
                else echo "Пользователь еще ничего не написал о себе";
              ?>
            </p>
          </div>
